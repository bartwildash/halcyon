  ...
  const timelineItems = useMemo(() => {
    ...
    return items
      .map(item => {
        ...
        const itemEnd = item.end ? new Date(item.end) : new Date(itemStart.getTime() + 7 * 24 * 60 * 60 * 1000);
...
function formatViewRange(range, mode) {
  ...
  switch (mode) {
    case 'day':
      return range.start.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
    case 'week':
